...
VARIANT_NAME_QUEST="edison-quest"
VARIANT_NAME=$VARIANT_NAME_BLANK
U_BOOT_BIN="u-boot-edison.bin"
HDD_IMG="edison-image-edison.hddimg"
ROOT_FS="edison-image-edison.ext4"
...
Update all software and restore board to its initial state.
 ...
 --quest       flash with quest as opposed to yocto linux
...
function flash-command-try {
	echo dfu-util -v -d ${USB_VID}:${USB_PID} $@
...
then
	...
	flash-command --alt u-boot0 -D "${ESC_BASE_DIR}/${U_BOOT_BIN}"
	...
	flash-command --alt u-boot-env1 -D "${VARIANT_FILE}"
        ...
        if [ $VARIANT_NAME == $VARIANT_NAME_QUEST ]; then
		echo "Flashing boot partition (kernel)"
		flash-command --alt boot -D "${ESC_BASE_DIR}/${HDD_IMG}" -R
        else
		echo "Flashing boot partition (kernel)"
		flash-command --alt boot -D "${ESC_BASE_DIR}/${HDD_IMG}"

		echo "Flashing rootfs, (it can take up to 5 minutes... Please be patient)"
		flash-command --alt rootfs -D "${ESC_BASE_DIR}/${ROOT_FS}" -R
	fi

	echo "Rebooting"
	echo "U-boot & Kernel System Flash Success..."
	if [ $VARIANT_NAME == $VARIANT_NAME_BLANK ] || [ $VARIANT_NAME == $VARIANT_NAME_QUEST ]; then
